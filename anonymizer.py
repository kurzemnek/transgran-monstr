...
def detect(header, rows, sample=600):
    ...
    for i, name in enumerate(header):
        ...
        if phone_full / total >= 0.5 or (key_col and phone_any / total >= 0.3):
            ...
            # Причина берётся от названия колонки: в «Комментарии» слабый детектор
            # имён срабатывает на «перезвонить позже», и человек справедливо
            # решает, что программа не понимает его данные.
            plan.append((name, 'drop', 'внутри текста встречаются контакты'
                         if (phone_any or email_any) else 'персональное поле по названию'))
...
def gui(preset=None):
    ...
    CHANGELOG = (
        ('1.5.1', '24 сентября 2026', (
            'причина решения по колонке больше не врёт: «перезвонить позже» не объявляется именем',
        )),
